...
/**
 * Tool: remove one or more labels from a GitLab issue.
 */
#[Tool(
  id: 'gitlab_api_remove_issue_label',
  label: new TranslatableMarkup('GitLab: remove label(s) from issue'),
  description: new TranslatableMarkup('Removes one or more comma-separated labels from a GitLab issue.'),
  ...
  input_definitions: [
    ...
    'labels' => new InputDefinition(
      ...
      description: new TranslatableMarkup('Comma-separated label names to remove.'),
      required: TRUE,
    ),
  ],
  output_definitions: [
...
final class RemoveIssueLabel extends GitlabIssueToolBase {
  ...
  protected function doExecute(array $values): ExecutableResult {
    ...
    try {
      $this->clientFactory->forProject($project)
        ->issues()
        ->update($project->getGitLabProjectId(), $iid, ['remove_labels' => $joined]);
    }
    catch (\Throwable $e) {
      return $this->failWithLog('Remove labels "@l" failed for issue #@iid: @msg', [
        '@l' => $joined,
        '@iid' => $iid,
        '@msg' => $e->getMessage(),
      ]);
    ...
    return ExecutableResult::success(
      $this->t('Removed labels "@l" from issue #@iid.', ['@l' => $joined, '@iid' => $iid]),
      ['issue_iid' => $iid]
    );
  }
