...
class HoursReportAccess extends AccessPluginBase {
  ...
  public function access(AccountInterface $account) {
    return $account->hasPermission('view work hours reports') || $account->hasPermission('manage user work hours') || $account->hasPermission('burndown comment on task');
  }
  ...
  public static function routeAccess(AccountInterface $account) {
    return AccessResult::allowedIf($account->hasPermission('view work hours reports') || $account->hasPermission('manage user work hours') || $account->hasPermission('burndown comment on task'))
      ->cachePerPermissions();
  }
