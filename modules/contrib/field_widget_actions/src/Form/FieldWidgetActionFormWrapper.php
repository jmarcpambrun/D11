...
class FieldWidgetActionFormWrapper extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $plugin_id = NULL, $context_data = []) {
    ...
    // This form bypasses the regular form processing pipeline: submissions
    // are detected and processed below, based on the user input and the
    // context data kept in the temp store. The form must therefore never be
    // served from the form cache, as the form builder would then process the
    // submission through the regular pipeline and redirect instead.
    ...
    // Instantiate the plugin with the configuration of the action that
    // opened the modal, if available.
    /** @var \Drupal\field_widget_actions\FieldWidgetFormActionInterface $plugin */
    $this->plugin = $this->manager->createInstance($plugin_id, $context_data['action_configuration'] ?? []);
    ...
    // form temporarily. When the form is being rebuilt, for example to
    // refresh the modal with refined content, the submission has already
    // been processed and must not run again.
    $user_input = $form_state->getUserInput();
    if (!empty($user_input['form_id']) && $user_input['form_id'] === 'field_widget_action_wrapper_form' && !$form_state->isRebuilding()) {
      ...
      // The cancel action discards the user's input, so it must close the
      // dialog even when the modal form would otherwise fail validation.
      // Route it straight to the plugin without validating first, otherwise a
      // modal form that adds its own validation could trap the user with no
      // way to dismiss the dialog.
      if (($user_input['_triggering_element_name'] ?? '') === 'cancel') {
  ...
  protected function getPluginInstance(FormStateInterface $form_state): FieldWidgetFormActionInterface {
    ...
    if (!$plugin instanceof FieldWidgetFormActionInterface) {
      ...
      if ($plugin_id) {
        // Configure the plugin instance with the configuration of the action
        // that opened the modal, if available.
        $context_data = $form_state->get('field_widget_action_context_data') ?? [];
        $plugin = $this->manager->createInstance($plugin_id, $context_data['action_configuration'] ?? []);
      }
    }
    return $plugin;
  }
