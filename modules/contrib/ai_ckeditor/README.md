...
When releasing a new version the built assets should be included in the source code. There is a script for that in
`scripts` folder - `tag-release.sh`. Use it to create a new tag with built assets, then use the drupal.org to add
new release of the module from this tag.

## Documentation

Full documentation is under [`docs/`](docs/) and published at
https://project.pages.drupalcode.org/ai_ckeditor.
