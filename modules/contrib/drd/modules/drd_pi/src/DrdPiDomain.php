...
class DrdPiDomain extends DrdPiEntity {
   ...
   * @return $this
   */
  public function setDetails(DrdPiCore $core, string $domain): self {
    $this->core = $core;
    $this->domain = $domain;
    return $this;
  }
   ...
   */
  private function determineUrl(bool $secure = TRUE): bool|string {
    ...
    $success = FALSE;
    try {
      $client = $this->httpClientFactory->fromOptions(['base_uri' => $url]);
      $response = $client->request('head', $url, $options);
    ...
    if (!$success) {
      $this->logging->debug('Can not connect to @url', [
        '@url' => $url,
      ]);
      if ($secure) {
