...
class IMAPService {
  ...
  public function connection() {
	...
	if (!function_exists('imap_open')) {
		throw new \Exception("IMAP functions are not available. Please ensure the PHP IMAP extension is installed and enabled.");
	}
    if ($this->imapConnection = @imap_open($imap_connection_string, $email_username, $imap_password)) {
      ...
      return TRUE;
        ...
        $error_message = is_array($imap_errors) ? implode("\n", $imap_errors) : "Unknown error";
        throw new \Exception("IMAP connection failed: " . $error_message); 
		return FALSE;
		}
	}	
  /**
