(function (Drupal, once) {
  Drupal.behaviors.ai_content_suggestions_toggle = {
    attach(context) {
      ...
      context
        .querySelectorAll('.toggle_content_suggestion_select')
        .forEach((container) => {
      ...
      once(
        ...
        '.toggle_content_suggestion_fields',
        context,
      ).forEach((element) => {
        element.addEventListener('click', function (e) {
          e.preventDefault();
          const formItem = this.closest('.form-item');
          const selectContainer = formItem.querySelector(
            '.toggle_content_suggestion_select',
          );
