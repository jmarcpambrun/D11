...
final class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    foreach ($this->pluginManager->getDefinitions() as $id => $definition) {
      ...
      $config = $content_suggestions_config->get('plugins')[$id] ?? [];
      if (!is_array($config)) {
        $config = [
          'model' => $config,
          'enabled' => TRUE,
    ...
    if (empty($available_plugins)) {
      ...
      foreach ($available_plugins as $plugin) {
        ...
        if ($plugin instanceof PluginFormInterface) {
          ...
          $form['plugins'][$plugin_id] = $plugin->buildConfigurationForm($subform, $subform_state);
        }
        else {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    foreach ($this->pluginManager->getDefinitions() as $id => $definition) {
      /** @var \Drupal\ai_content_suggestions\AiContentSuggestionsInterface $plugin */
      if ($plugin = $this->pluginManager->createInstance($id, [])) {
        // Check if plugin is available (not confuse with enabled).
        if (!$plugin->isAvailable()) {
          continue;
        }
        if ($plugin instanceof PluginFormInterface) {
          if (isset($form['plugins'][$id])) {
            $subform = &$form['plugins'][$id];
            $subform_state = SubformState::createForSubform($subform, $form, $form_state);
            $plugin->submitConfigurationForm($subform, $subform_state);
            $form_state->setValue(['plugins', $id], $plugin->getConfiguration());
          }
        }
        elseif (method_exists($plugin, 'saveSettingsForm')) {
          $value = $form_state->getValue(['plugins', $id]);
          // Ensure $value is an array before accessing keys.
          if (is_array($value) && !empty($value[$id . '_enabled'])) {
            $values['plugins'][$id] = $value[$id . '_model'];
          }
    ...
    $values = array_merge($form_state->getValue('plugins', []), $values);
