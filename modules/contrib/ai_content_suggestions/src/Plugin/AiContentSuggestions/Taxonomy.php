 ...
 * )
 */
final class Taxonomy extends AiContentSuggestionsPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['prompt_open'] = [
      ...
      '#required' => TRUE,
      '#default_value' => $config['prompt_open'],
      '#states' => [
        'visible' => [
          ':input[name="plugins[' . $this->getPluginId() . '][enabled]"]' => ['checked' => TRUE],
        ],
      ],
      '#parents' => array_merge($form['#parents'], ['prompt_open']),
    ];
    $form['prompt_from_voc'] = [
      '#title' => $this->t('Suggest taxonomy prompt (limited to vocabulary)'),
      '#type' => 'ai_prompt',
      '#prompt_types' => ['suggest_vocabulary'],
      '#required' => TRUE,
      '#default_value' => $config['prompt_from_voc'],
      '#states' => [
        'visible' => [
          ':input[name="plugins[' . $this->getPluginId() . '][enabled]"]' => ['checked' => TRUE],
        ],
      ],
      '#parents' => array_merge($form['#parents'], ['prompt_from_voc']),
    ];
    return $form;
  }
  ...
  public function updateFormWithResponse(array &$form, FormStateInterface $form_state): void {
    if ($value = $this->getTargetFieldValue($form_state)) {
      ...
      if ($this->getFormFieldValue('use_source_vocabulary', $form_state)) {
        ...
        // Build our prompt.
        $tax_prompt_id = $config['prompt_from_voc'];
        ...
        $tax_prompt_id = $config['prompt_open'];
        $tax_prompt = AiPrompt::load($tax_prompt_id)->getPrompt();
        $prompt = '';
        if (!empty($tax_prompt)) {
          $prompt = $tax_prompt;
